...
Message::Message(const std::shared_ptr<MessageBuilderBase>& builder) : builder_(builder) {}
...
uint64_t Message::When() {
  return when_;
}

void Message::SetWhen(const uint64_t when) {
  when_ = when;
}
...
bool Message::Join(const int64_t timeout_ms) {
  return event_.Join(timeout_ms);
}
