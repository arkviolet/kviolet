 ...
 * */
#define KVIOLET_ASSERT_CPP(value)  { throw std::runtime_error(value);};

/**
 * assert false throw std::runtime_error
 * */
#define KVIOLET_ASSERT_CPP_COND(condition, value)  { if(!(condition)) {throw std::runtime_error(value);} };
