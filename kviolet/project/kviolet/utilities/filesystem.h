...
namespace kviolet
{
    ...
    public:
        ...
        ///读取文件内容
        static std::size_t ReadFile(const std::string &path,
        ...
        ///遍历文件
        static void TraverseFile(const std::string &path, std::vector<std::string> &container,
                                 bool subdirectory = false);

        ///遍历文件
        static void TraverseFile(const std::string &path,
                                 std::vector<std::string> &container, const std::regex &rule,
                                 bool subdirectory = false);

        ///遍历文件
        static std::vector<std::string> TraverseFile(const std::string &path,
                                                     bool subdirectory = false);

        ///遍历文件
        static std::vector<std::string> TraverseFile(const std::string &path,
                                                     const std::regex &rule, bool subdirectory = false);

        ///遍历文件
        static void TraverseDirectory(const std::string &path,
                                      std::vector<std::string> &container, bool subdirectory = false);

        ///遍历文件
        static void TraverseDirectory(const std::string &path, std::vector<std::string> &container,
                                      const std::regex &rule, bool subdirectory = false);
