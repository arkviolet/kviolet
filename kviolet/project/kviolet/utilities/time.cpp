...
namespace kviolet
{
    ...
    std::string Time::FormatTimeString(std::time_t seconds, std::time_t timeZone, const char *format)
    {
        ...
        snprintf(str, sizeof(str), format, now.tm_year + 1900, now.tm_mon + 1, now.tm_mday, now.tm_hour, now.tm_min,
                 now.tm_sec);
    ...
    SystemClockTimePoint Time::TimePoint(std::time_t time)
    {
        return std::chrono::time_point_cast<SystemClockDuration>(
                std::chrono::time_point<SystemClockType, std::chrono::seconds>(std::chrono::seconds(time)));
    }
