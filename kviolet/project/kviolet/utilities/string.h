...
namespace kviolet
{
    ...
    public:
        ...
        static std::string Erase(const std::string &value,
                                 const std::function<bool(char)> &function, std::size_t index = 0);


        ///字符串替换字符
        static void Replace(std::string &value, const std::string &src, const std::string &dst, std::size_t index = 0);

        static std::string Replace(const std::string &value, const std::string &src,
                                   const std::string &dst, std::size_t index = 0);

        static void ReplaceAll(std::string &value, const std::string &src,
                               const std::string &dst, std::size_t index = 0);

        static std::string ReplaceAll(const std::string &value, const std::string &src,
                                      const std::string &dst, std::size_t index = 0);
        ...
        ///按字符串拆分
        static std::size_t Split(const std::string &value, const std::string &key,
                                 std::vector<std::string> &container, bool keepEmpty = false);

        static std::vector<std::string> Split(const std::string &value, const std::string &key, bool keepEmpty = false);


        ///按行拆分
        static std::size_t SplitLines(const std::string &value, std::vector<std::string> &container,
                                      bool keepEnter = false);
